...
export default class NeoExpressTerminal {
  ...
  constructor(private readonly shellPath: string, private readonly shellArgs: string[]) {
  ...
  open(initialDimensions: vscode.TerminalDimensions | undefined) {
    ...
    this.process = childProcess.spawn(
      this.shellPath,
      this.shellArgs.filter((a) => a !== ""),
      {
        cwd: workspaceFolder() || process.cwd(),
        shell: true,
      }
    );
    this.process.stdout.on("data", (d) => this.writeEmitter.fire("\r\n" + d.toString().trim()));
    this.process.stderr.on("data", (d) => this.writeEmitter.fire("\r\n" + d.toString().trim()));
